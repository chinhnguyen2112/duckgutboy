...
if (isset($accessToken)) { // đã lấy được token, đăng nhập thành công
    ...
    // echo $iduser;
        ...
        $db->query("INSERT INTO `accounts` (username,name,email,active,cash,luotquay_free,point,admin,block,time,source) VALUES ('$iduser','$name','$email',1,0,'$count',0,0,0,'$date_current','$_source')");
    } else {
        ...
        if ($date_check >= '11:00:00' && $date_check < '12:00:00') { // nếu đang vào khung giờ vàng đâu tiên
            ...
            if (count($check_his) > 0) { // nếu đã có data
                ...
                if (date('d-m-Y', $check_his['created_at']) < date('d-m-Y', time())) { // nếu hôm nay chưa nhận
                    ...
                    if (date('H:i:s', $check_his['created_at']) < '20:00:00' && date('H:i:s', $check_his['created_at']) >= '21:00:00') { // nếu hôm nay đã nhận khung giờ vàng thứ hai
                        $event = 0; // 
