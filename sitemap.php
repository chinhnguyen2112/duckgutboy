...
if ($_GET['type'] == 'home') {
    ...
    $doc->appendChild($doc->createProcessingInstruction('xml-stylesheet', 'type="text/xsl" href="https://duckgutboy.com/assets/css/sanacc/css_sitemap.xsl"'));
    ...
    foreach ($db->fetch_assoc($sql, 0) as $key => $val) {
        ...
        $name->appendChild(
            $doc->createTextNode('https://duckgutboy.com/' . $val['name'])
        );
    ...
    $sql = "SELECT id,alias,updated_at FROM baiviet ORDER BY id DESC";
    $blog = $db->fetch_assoc($sql, 0);
    $count = count($blog);
    $page = ceil($count / 200);
    for ($i = 1; $i <= $page; $i++) {
        $check_page = ($i - 1) * 200;
        $sql_limit = "SELECT id,alias,updated_at FROM baiviet ORDER BY id DESC LIMIT {$check_page}, 200";
        ...
        foreach ($blog_limit as $val) {
            ...
            $name = $doc->createElement("loc");
            $name->appendChild(
                $doc->createTextNode('https://duckgutboy.com/' . $val['alias'] . '/')
            );
            $url->appendChild($name);
    ...
    $sql = "SELECT id_post,date_posted FROM posts ORDER BY id_post DESC";
    $blog = $db->fetch_assoc($sql, 0);
    $count = count($blog);
    $page = ceil($count / 200);
    for ($i = 1; $i <= $page; $i++) {
        $check_page = ($i - 1) * 200;
        $sql_limit = "SELECT id_post,date_posted FROM posts ORDER BY id_post DESC LIMIT {$check_page}, 200";
        $blog_limit = $db->fetch_assoc($sql_limit, 0);
        $doc = new DOMDocument("1.0", "utf-8");
        $doc->formatOutput = true;
        $r = $doc->createElement("urlset");
        ...
        foreach ($blog_limit as $val) {
            ...
            $name = $doc->createElement("loc");
            $name->appendChild(
                $doc->createTextNode('https://duckgutboy.com/tai-khoan-' . $val['id_post'] . '/')
            );
            $url->appendChild($name);
            $changefreq = $doc->createElement("changefreq");
            $changefreq->appendChild(
